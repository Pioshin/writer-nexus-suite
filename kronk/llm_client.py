...
# Import shared config manager (UnifiedConfigManager vive in ADAM-suite/)
...
class LLMClient:
    def __init__(self, provider=None, model=None, base_url=None, api_key=None, ctx=None, timeout=None, keep_alive=None):
        # Defaults (usati solo come fallback se UnifiedConfigManager non è disponibile)
        self.provider = "ollama"
        self.model = "adam:latest"
        ...
        self.config_manager = UnifiedConfigManager() if UnifiedConfigManager else None

        # Carica da shared config (la stessa di IODA e BKA)
        self.load_config()

        # Override espliciti se forniti (rari — usati da app.py al boot)
    ...
    def load_config(self):
        """Load configuration from Shared UnifiedConfigManager (stessa di IODA/BKA)."""
        if not self.config_manager:
            return
        try:
            ai_config = self.config_manager.get_ai_config()
            self.provider = ai_config.get("provider", self.provider)
            self.model = ai_config.get("model", self.model)
            self.base_url = ai_config.get("url", self.base_url)
            self.api_key = ai_config.get("api_key", self.api_key)
            self.ctx = ai_config.get("ctx", self.ctx)
            self.timeout = ai_config.get("timeout", self.timeout)
            self.keep_alive = ai_config.get("keep_alive", self.keep_alive)
            # Aggiorna endpoint Ollama (potrebbero non esistere ancora al primo init)
            self.ollama_api_chat = f"{self.base_url}/api/chat"
            self.ollama_api_generate = f"{self.base_url}/api/generate"
    ...
    def save_config(self):
        """Salva la config corrente nel manager condiviso."""
        if not self.config_manager:
            return
        try:
    ...
    def chat(self, messages, stream=False):
        """Generic chat method."""
        # Ricarica config condivisa: cambiamenti fatti da IODA o BKA si propagano subito
        self.load_config()
